...
<script>
import { getAllCountEnterpriseByYear } from "../../../api/dashboard";
export default {
  name: "EnterpriseChart",
  components: {},
  data() {
    return {
      year: "",
  ...
  methods: {
    fetch() {
      getAllCountEnterpriseByYear(this.form.year).then((result) => {
  ...
  watch: {
    year() {
      getAllCountEnterpriseByYear(this.year).then((result) => {
        this.series[0].data = result.data[0].data;
        if (this.year == "") {
